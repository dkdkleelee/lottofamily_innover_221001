...
function admin_referer_check($return=false)
{
    ...
    if( $p['path'] && ! preg_match( '/\/'.preg_quote(G5_MANAGER_DIR).'\//i', $p['path'] ) ){
        $msg = '올바른 방법으로 이용해 주십시오';
    }
...
if (!$member['mb_id'])
{
    alert('로그인 하십시오.', G5_BBS_URL.'/login.php?url=' . urlencode(G5_MANAGER_URL));
}
...
unset($menu);
unset($amenu);
$tmp = dir(G5_MANAGER1_PATH);
while ($entry = $tmp->read()) {
    if (!preg_match('/^admin.menu([0-9]{3}).*\.php$/', $entry, $m))
        continue;  // 파일명이 menu 으로 시작하지 않으면 무시한다.

    $amenu[$m[1]] = $entry;
    include_once(G5_MANAGER1_PATH.'/'.$entry);
}
@ksort($amenu);
