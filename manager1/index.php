...
$ym = date('Y-m');
$data = $termService->getSellStatistics($ym, $_SESSION['ss_mb_id']);

foreach($data as $key => $val) {
	$data_st_list['label'][] = $key;
	$data_st_list['var'][] = $val['cnt'] > 0 ? $val['cnt'] : 0;
}

$pre_ym = date('Y-m', strtotime($ym."-01 -1 month"));
$data = $termService->getSellStatistics($pre_ym, $_SESSION['ss_mb_id']);

foreach($data as $key => $val) {
	$data_st_pre_list['label'][] = $key;
	$data_st_pre_list['var'][] = $val['cnt'] > 0 ? $val['cnt'] : 0;
}

$cur_legend = $ym;
$pre_legend = date("Y-m", strtotime($ym."-01 -1 month"));
...
<section>
	<div class="btitle">
		<div style="padding-left:10px"><i class="fa fa-folder-o"></i> 영업현황</div>
	</div>
...
var chartOptions = {
		...
		legendTemplate: "<ul class=\"<%=name.toLowerCase()%>-legend\" style=\"list-style:none;\"><% for (var i=0; i<datasets.length; i++){%><li style=\"margin:5px\"><span style=\"background-color:<%=datasets[i].fillColor%>;    margin:3px;padding: 2px 7px;\"></span><%if(datasets[i].label){%><%=datasets[i].label%><%}%></li><%}%></ul>",
	...
	window.onload = function(){
		...
		window.myChart = new Chart(ctx).Bar(ChartData, chartOptions);
