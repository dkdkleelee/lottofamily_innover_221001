...
$g5['title'] = '<i class="fa fa-play"></i> 정상회원 관리';
...
<!--        <span class="help_text"><i class="fa fa-question-circle fa-lg f_blue"></i></span>-->
    ...
    <div class="search_box ar">
        <form name="searchForm" method="get" action="?">
			...
			<div>
				<ul class="mem-count">
					<li class="<?=($_GET['s_sg_no'] == '') ? 'on' : ''?>"><a href="<?=$_SERVER['PHP_SELF']?>?s_sg_no=">총 <strong><?=number_format($tot_count)?></strong>명회원</a></li>
			...
			<div>
				...
				<select name="s_sg_no" class="frm_input" onChange="document.searchForm.submit()">
    ...
    <tr id="pos_<?=$row['mb_no']?>" class="<? if(count($row_service) > 0) { ?>on<? } ?>">
		...
		<td class="al pl5" <?=($row_memo[0]['mo_memo']) ? 'data-tooltip="<span style=\'font-weight:bold\'>['.$row_memo[0]['mb_name'].' / '.$row_memo[0]['mo_datetime']."]</span><br />".nl2br($row_memo[0]['mo_memo']).'"' : ''?>><?=($row_memo[0]['mo_memo']) ? '<span >'.cut_str($row_memo[0]['mo_memo'],25)."</span><br /><span style='font-weight:bold'>[".$row_memo[0]['mo_datetime']."]</span>" : '상담이전'?></td>
	...
	<div class="flex m-5">
		...
		<button type="button" class="as-btn small green" onclick="stopUsers()"><i class="fa fa-pause"></i> 선택회원 이용중지</button>
