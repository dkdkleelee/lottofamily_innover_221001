...
$g5['title'] = '<i class="fa fa-stop"></i> 탈퇴회원 관리';
...
<!--        <span class="help_text"><i class="fa fa-question-circle fa-lg f_blue"></i></span>-->
    ...
    <tr id="pos_<?=$row['mb_no']?>" class="<? if(count($row_service) > 0) { ?>on<? } ?>">
		...
		<td class="al pl5" <?=($row_memo[0]['mo_memo']) ? 'data-tooltip="<span style=\'font-weight:bold\'>['.$row['mb_update_date']."]</span><br />".nl2br($row_memo[0]['mo_memo']).'"' : ''?>><?=($row_memo[0]['mo_memo']) ? '<span >'.cut_str($row_memo[0]['mo_memo'],25).'</span>' : '상담이전'?></td>
