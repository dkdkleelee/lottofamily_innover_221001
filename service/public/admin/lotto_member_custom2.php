...
//$tot_count = $user->getUserWithNoLoginCount($inning_arr[0]['inning']+1, 4);
$user_list = $user->getUserListWithNoLogin($_GET['page'], $list_url, $inning_arr[0]['inning']+1, 4);


auth_check($auth[$sub_menu], 'w');

$g5['title'] = '<i class="fa fa-play"></i> 4X';
...
<!--        <span class="help_text"><i class="fa fa-question-circle fa-lg f_blue"></i></span>-->
	...
	<div class="flex m-5">
		...
		선택한 회원들의 <label style="background-color:#4f95ff;color:#fff;padding:5px 4px"> <?=$inning_arr[0]['inning']+1?>회차</label>
		 발급된 당첨번호 중에 1개를 <label style="background-color:#6b8225;color:#fff;padding:5px 4px">4등 당첨번호</label> 로 
		<button type="button" class="as-btn small green" onclick="setNumberNoLogin('<?=$inning_arr[0]['inning']+1?>', 4)"><i class="fa fa-refresh"></i> 변경</button> 하고, 당첨문자를 발송합니다.
		</div>
		<div class="flex-3">
