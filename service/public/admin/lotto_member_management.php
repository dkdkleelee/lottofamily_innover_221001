...
$g5['title'] = $_GET['mb_id'] ? $data['mb_name']."(".$data['mb_id'].")회원 정보 수정" : "회원 등록";
...
<div class="content_wrap">
	<div>
		<span class="btitle"><i class="fa fa-folder-open-o"></i> <?=$g5['title']?></span>
	</div>
	<form name="member_form1" id="member_form1" method="post" action="./lotto_member_process.php">
	...
	<tr>
		...
		<td width="40%">
			...
			<span id="group_info">
			<select name="mg_no" id="mg_no" class="frm_input">
				<option value="">팀선택</option>
			<? foreach($group_arr as $key => $value) { ?>
				<option value="<?=$value['mg_no']?>" <?=$data['mg_no'] == $value['mg_no'] ? 'selected' : ''?>><?=$value['mg_name']?></option>
			<? } ?>
			</select>
			</span>
		</td>
	...
	<tr>
		<th width="140">이름(닉네임)</th>
		<td>
			<input type="text" name="mb_name" class="frm_input" value="<?=$data['mb_name']?>" placeholder="이름" required><?=$data['mb_nick'] ? '('.$data['mb_nick'].')' : ''?>
		</td>
	...
	<tr>
		...
		<td>
			<?=$data['mb_today_login']?> (<span class="fwb fcg"><?=$data['lastLoginDays']?>일전</span>) : IP - <?=$data['mb_login_ip']?>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			...
			<select name="mb_hp1" id="reg_mb_hp1" class="frm_input" style="width:65px" required>
	...
	<tr>
		...
		<td width="40%">
			<select name="issue_count" id="issue_count" class="frm_input">
			<? for($i=5; $i<=45; $i+=5) { ?>
				<option value="<?=$i?>"><?=$i?>개</option>
			<? } ?>
			</select>
			<button type="button" class="as-btn tiny blue" onclick="addNumbersToQueue()"><i class="fa fa-send"></i> 추가발급</button>
		</td> -->
	</tr>
	<tr>
		...
		<td colspan="3">
		   ...
		   조정 : <select name="mb_extract_per_week" id="mb_extract_per_week" class="frm_input">
				...
				<option value="5" <?=$data['mb_extract_per_week'] == 5 ? 'selected' : ''?>>5개/주</option>
				<option value="10" <?=$data['mb_extract_per_week'] == 10 ? 'selected' : ''?>>10개/주</option>
				<option value="15" <?=$data['mb_extract_per_week'] == 15 ? 'selected' : ''?>>15개/주</option>
				<option value="20" <?=$data['mb_extract_per_week'] == 20 ? 'selected' : ''?>>20개/주</option>
				<option value="25" <?=$data['mb_extract_per_week'] == 25 ? 'selected' : ''?>>25개/주</option>
	...
	<div style="width:100%;text-align:center;padding-bottom:30px">
		<button type="button" onclick="document.member_form1.submit()" name="submit_btn1" class="as-btn small blue"><i class="fa fa-check"></i> 확인</button>
		<button type="button" class="as-btn small red" onclick="window.close();"><i class="fa fa-close"></i> 닫기</button>
	</div>
	</form>



	<form name="member_form2" id="member_form2" method="post" action="./lotto_member_process.php" enctype="multipart/form-data" onsubmit="return formSubmit(this);">
			...
			<div style="width:100%;height:200px;overflow-y:scroll;border:1px solid #eeeeee;margin-top:5px">
				...
				?>
					<li style="padding:5px;border-bottom:1px dotted #e8e8e8">
						<?=nl2br($value['mo_memo'])?><div style="color:#6666ff"><?=$value['mo_datetime']?> [<span style="font-weight:bold"><?=$value['mb_name']?></span>] 
	...
	<!--tr>
		<th width="140">이미지</th>
		<td colspan="1">
			<input type="file" name="simul_img" id="simul_img">
		
		<? if($user->getSimul($_GET['mb_id'])) { ?>
	
			&nbsp;&nbsp;
			<button type="button" class="as-btn small red" onclick="deleteSimul('<?=$_GET['mb_id']?>');">기존이미지삭제
			</button>
		<? } ?>
		</td>
	</tr-->
	...
	<br><br />
	<div style="width:100%;text-align:center;padding-bottom:30px">
		<button type="button" onclick="document.member_form2.submit()" name="submit_btn2" class="as-btn small blue"><i class="fa fa-check"></i> 확인</button>
		<button type="button" class="as-btn small red" onclick="window.close();"><i class="fa fa-close"></i> 닫기</button>
	</div>
	</form>
	<!--// 회원기본정보 및 상담정보 -->
...
<? if($data['mb_id']) { ?>
	...
	<tr>
		<th>서비스</th>
		<th>결제방법</th>
		<th class="pay_fields pay_agency pay_manual">카드정보</th>
		<th class="pay_fields pay_agency pay_manual">추가정보(payUp결제용)</th>
		<th class="pay_fields pay_account pay_pg2">입금자명</th>
		<th class="pay_fields pay_account pay_pg2">입금계좌</th>
	...
	<tr>
		<td>
			...
			금액수정: <input type="text" name="new_price" id="new_price" placeholder="숫자만 입력" class="frm_input" style="width:150px;margin:2px">원
			<!-- <br />
		...
		<td>
			...
			?>
			<select name="pay_method" id="pay_method" class="frm_input" onChange="setPayMethod(this)">
			<? foreach($pay_method_arr as $key => $value) { ?>
				<option value="<?=$key?>"><?=$value?></option>
			<? } ?>
			</select>
		</td>
		<td class="pay_fields pay_agency pay_manual">
			...
			<select name="installment" class="form-control frm_input">
				...
				<option value="2">02개월</option>
				<option value="3">03개월</option>
				<option value="4">04개월</option>
				<option value="5">05개월</option>
				<option value="6">06개월</option>
				<option value="7">07개월</option>
				<option value="8">08개월</option>
				<option value="9">09개월</option>
		...
		<td class="pay_fields pay_agency pay_manual">
			<input type="text" name="sb_ay_birth" placeholder="생년월일(820903)" class="form-control frm_input" style="width:70px;" maxlength="6" autocomplete="false" ><br />
			비밀번호 앞 2자리 : <input type="password" name="sb_ay_f2code" placeholder="" class="form-control frm_input" style="width:30px;" maxlength="2" autocomplete="false"  readonly onfocus="this.removeAttribute('readonly');">**
		</td>
		<td class="pay_fields pay_account pay_pg2">
    		<input type="text" name="payer_name" class="frm_input" style="width:80px">
		</td>
		<td class="pay_fields pay_account pay_pg2">
	...
	<tr>
        ...
        <td>
			총: <?=number_format($row['sb_total_price'])?>원<? if($row['sb_pay_method'] == 'agency') { ?> <? } ?><br />
			결제금액: <?=number_format($row['sb_price'])?>원
			<!-- 미수금: <?=number_format($row['sb_not_paid_price'])?>원 -->
		
		</td>
		<td id="pay_status_<?=$row['sb_no']?>">
    		<?=$termService->config['pay_method'][$row['sb_pay_method']]?>
			<? if($row['sb_pay_method'] == 'agency' && $row['sb_pay_status'] == 'N') { ?>
				<!-- <button type="button" class="as-btn tiny blue" onclick="agencyPay2('<?=$row['sb_no']?>','futureSurf')"><i class="fa fa-credit-card"></i> FS결재진행</button> -->

				<!-- <button type="button" class="as-btn tiny blue" onclick="agencyPay3('<?=$row['sb_no']?>','futureSurf')"><i class="fa fa-credit-card"></i> FS결재진행(사용금지)</button> -->
				<!--button type="button" class="as-btn tiny blue" onclick="agencyPay3('<?=$row['sb_no']?>','payUp')"><i class="fa fa-credit-card"></i> PU결제진행</button-->
		
				<!--button type="button" class="as-btn tiny blue" onclick="agencyPay3('<?=$row['sb_no']?>', 'PG2')"><i class="fa fa-credit-card"></i> PG2결제진행</button-->

				<button type="button" class="as-btn tiny blue" onclick="agencyPay3('<?=$row['sb_no']?>','SHCard2')"><i class="fa fa-credit-card"></i> AB결제진행</button>
			
			<? } else if($row['sb_ay_pg'] == 'payUp' && $row['sb_pay_status'] == 'Y') { ?>
			
				<button type="button" class="as-btn tiny blue" onclick="agencyPayCancel('<?=$row['sb_no']?>','payUp')"><i class="fa fa-credit-card"></i> PU결재취소</button>
			<? } ?>
		</td>
		<td >
    		<?=$row['sb_pay_name']?>
		...
		<td id="agree_status_<?=$row['sb_no']?>">
			...
			<? } else { ?>
			<button type="button" class="as-btn tiny blue" onclick="sendProvisionSms('<?=$row['sb_no']?>')"><i class="fa fa-comment"></i> 약관전송</button>
			<? } ?>

			<? if($row['sb_pay_method'] == 'account' && $row['sb_pay_status'] == 'N') { ?>
			<button type="button" class="as-btn tiny blue" onclick="sendAccountSms('<?=$row['sb_no']?>')"><i class="fa fa-comment"></i> 계좌정보전송</button>
			<? } ?>
		</td>
		<td>
    		...
    		<form name="service_buy_<?=$row['sb_no']?>" method="post" action="./term_service_buy_process.php">
        		...
        		<select name="sb_pay_status" class="frm_input" onChange="document.service_buy_<?=$row['sb_no']?>.submit()">
					...
					<option value="C" <?=$row['sb_pay_status'] == 'C' ? 'selected' : ''?>>결제취소</option>
        		</select>
    		</form>
		...
		<td>
			<button type="button" class="as-btn tiny red" onclick="deleteData('<?=$row['sb_no']?>')"><i class="fa fa-close"></i> 삭제</button>
...
?>
		<form name="serviceUseForm_<?=$row['su_no']?>" method="post" action="./term_service_use_process.php">
		<input type="hidden" name="proc" value="updateServiceEndDate">
		...
		<tr>
			...
			<td style="text-align:center">
			   <input type="text" name="su_enddate" value="<?=substr($row['su_enddate'],0,10)?>" class="frm_input datepicker">
			</td>
			<td style="text-align:center">
			   <b><?=$row['leftDays']?></b>일
			</td>
			...
			<td style="text-align:center">
				<button type="submit" class="as-btn small blue" ><i class="fa fa-refresh"></i> 수정</button>
			</td>
		</tr>
		</form>
<? } ?>
...
*/
	...
	<form name="deleteSimul_form" id="deleteSimul_form" method="POST" action="./lotto_member_process.php">
		<input type="hidden" name="proc" id="proc" value="deleteSimul">
		<input type="hidden" name="mb_id" id="mb_id" value="<?=$_GET['mb_id']?>">
		<input type="hidden" name="return_url" value="<?=$return_url?>">
	</form>
...
function formSubmit(f) {
	...
	var extArray = new Array('jpg','png','JPG','PNG');
	var path = document.getElementById("simul_img").value;
	if(path == "") {
		return true;
	}
...
function deleteSimul(id) {
	...
	var f = document.deleteSimul_form;
	if (!id) {
		return false;
	}
	else {
...
// 결제신청 - 새창
function agencyPay3(sb_no, pg) {

	var f = document.agencyPayForm;
	f.proc.value = "agencyPayUni";
	f.pg.value = pg;
	f.sb_no.value = sb_no;
	f.return_url.value = "close";
...
function agencyPayCancel(sb_no, pg) {
	var f = document.agencyPayForm;
	f.proc.value = "agencyPayCancelUni";
	f.pg.value = pg;
	f.sb_no.value = sb_no;
	f.return_url.value = "close";
...
function openUserWinList(mb_id) {
	window.open("./lotto_member_win_list.php?id="+mb_id, 'win_list', 'width=1000, height=800, scrollbars=yes');
}
