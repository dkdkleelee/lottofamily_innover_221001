...
//▶ get list data
$data = $user->getAlertMemoList('', $_GET['page'], $list_url);
...
$data_undone = $user->getAlertMemos('', 0, $year, $month);
$data_done = $user->getAlertMemos('', 1, $year, $month);
...
<div class="content_wrap">
	...
	<tr class="<?=($row['mb_cousult_status'] == '예약') ? 'rev' : ''?>">
        ...
        <td>
    		<?=$row['mo_schedule_done_datetime']?><?=$row['mo_schedule_done_datetime'] ? ' <div style="display:inline;width:15px;background:#76ac41;border-radius:30px;padding:2px;color:#fff"><i class="fa fa-check"></i></div>' : ''?>
		</td>
		<td>
