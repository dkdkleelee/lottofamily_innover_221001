...
// 목록
$user = new User();
$_GET['s_not_distributed'] = true;
$user_list = $user->getUserListWithService($_GET['page'], $list_url);
...
$tot_count = $user->getUserTotalCount();

$channel_group = $user->getChannelGroup('', "(mb_tm_id IS NULL OR mb_tm_id='')");

$media_group = $user->getMediaGroup('', $_GET['s_mb_channel'], "mb_tm_id IS NULL OR mb_tm_id=''");
...
<!--        <span class="help_text"><i class="fa fa-question-circle fa-lg f_blue"></i></span>-->
    ...
    <div class="search_box ar">
        <form name="searchForm" method="get" action="?">
			...
			<div>
				<ul class="mem-count">
					<li class="<?=($_GET['s_sg_no'] == '') ? 'on' : ''?>"><a href="./lotto_member.php?s_sg_no=">총 <strong><?=number_format($user_list['total_count'])?></strong>명회원</a></li>
					<!-- <? foreach($termService->service_grade as $key => $value) { ?>
					<li class="<?=($_GET['s_sg_no'] == $key) ? 'on' : ''?>"><a href="./lotto_member.php?s_sg_no=<?=$key?>"><?=$value?> (<?=$serviceCount[$value]?>명)</a></li>
					<? } ?>
					<li class="<?=($_GET['s_sg_no'] == 'normal') ? 'on' : ''?>"><a href="./lotto_member.php?s_sg_no=normal">일반 (<?=$serviceCount['일반']?>명)</a></li> -->
				</ul>
			</div>
			...
			<div>
				...
				<select name="s_mb_media" class="frm_input" onChange="document.searchForm.submit()">
					<option value="">광고매체</option>
						<option value="" <?=$_GET['s_mb_media'] == '' ? 'selected' : ''?>>전체</option>
					<? 
    ...
    <tr id="pos_<?=$row['mb_no']?>">
		...
		<td>
			...
			<button type="button" class=" as-btn tiny <?=$row['mb_level'] >= '3' ? 'blue' : 'green'?>" style="color:white"><?=$user->getUserType()[$row['mb_level']]?></button>
		...
		<td class="al pl5" <?=($row_memo[0]['mo_memo']) ? 'data-tooltip="'.nl2br($row_memo[0]['mo_memo']).'"' : ''?>><?=($row_memo[0]['mo_memo']) ? '<span >'.cut_str($row_memo[0]['mo_memo'],25).'</span>' : '상담이전'?></td>
        ...
        <td>
			...
			<button type="button" id="btn<?echo $btnCount?>" class="as-btn small blue" onclick="setCurrent('<?=$row['mb_no']?>');showManageWin('<?=$row['mb_id']?>');highlightBtn('btn<?echo $btnCount?>');">
				<i class="fa fa-eye"></i> 관리
			</button>
			<?$btnCount++?>

			<button type="button" id="btn<?echo $btnCount?>" class="as-btn small blue" onclick="showSendManageWin('<?=$row['mb_id']?>');highlightBtn2('btn<?echo $btnCount?>');">
                                <i class="fa fa-eye"></i> 발급관리
                        </button>
			<?$btnCount++?>

			<!-- <button type="button" class="as-btn small blue" onclick="setCurrent('<?=$row['mb_no']?>');location.href='./lotto_member_regist.php?<?=$param?>&mb_id=<?=$row['mb_id']?>&url=<?=urlencode($return_url)?>&pos=pos_<?=$row['mb_no']?>'">
				<i class="fa fa-check"></i> 수정
			</button> -->
			<? if($row['mb_id'] != 'admin') { ?>
				<? if($row['mb_leave_date'] == '') { ?>
				<!-- <button type="button" class="as-btn small red" onclick="unsubscribeUser('<?=$row['mb_id']?>')">
					<i class="fa fa-check"></i> 탈퇴
				</button> -->
				<? } else { ?>
	...
	<div class="flex m-5">
		...
		<button type="button" class="as-btn small red" onclick="deleteSelectedUsers()"><i class="fa fa-close"></i> 선택회원 삭제</button>
		</div>
		<div class="flex-3">
			...
			<button type="button" class="as-btn small blue" onclick="excel_upload();"><i class="fa fa-upload"></i> 회원 엑셀업로드</button>
			<button type="button" class="as-btn small green" onclick="showManageWin('');"><i class="fa fa-plus"></i> 회원등록</button>
		</div>
	</div>
	<div class="flex m-5">
		<div class="flex-1">
		<label style="background-color:#4f95ff;color:#fff;padding:5px 4px">선택한 회원의 지정TM을</label>
		<select name="c_mb_charger_tm" id="c_mb_charger_tm" class="frm_input">
			<option value="">담당TM선택</option>
				<option value="na" <?=$_GET['s_mb_charger_tm'] == 'na' ? 'selected' : ''?>>미지정</option>
			<? for($i=0; $i<count($tm_list); $i++) { ?>
				<option value="<?=$tm_list[$i]['mb_id']?>" <?=$tm_list[$i]['mb_id'] == $_GET['s_mb_charger_tm'] ? 'selected' : ''?>><?=$tm_list[$i]['mb_id']?>(<?=$tm_list[$i]['mb_name']?>)</option>
			<? }?>
		</select>로 <button type="button" class="as-btn small blue" onclick="changeChargerTMs()"><i class="fa fa-refresh"></i> 변경</button>
		</div>

		<div class="flex-1 ar">
		<label style="background-color:#6b8225;color:#fff;padding:5px 4px">선택한 회원의 상담상태를</label>
		<select name="c_mb_cousult_status" id="c_mb_cousult_status" class="frm_input">
			<option value="">상담상태</option>
			<? foreach($user->getConsultStatus() as $key => $value) { ?>
			<option value="<?=$key?>"><?=$value?></option>
			<? } ?>
		</select>로 <button type="button" class="as-btn small blue" onclick="changeConsultStatus()"><i class="fa fa-refresh"></i> 변경</button>
		</div>

	</div>

	<div class="flex m-5 ar">
		 <div class="flex-1 ar">
                <label style="background-color:#BE33FF;color:#fff;padding:5px 4px">선택한 회원의 발급요일을</label>
                <select name="c_mb_extract_weekday" id="c_mb_extract_weekday" class="frm_input">
			...
			<option value="1">월</option>
                ...
                </select>로 <button type="button" class="as-btn small blue" onclick="changeExtractWeekday()"><i class="fa fa-refresh"></i> 변경</button>
                </div>
	</div>

	<div class="flex m-5 ar">
		<div class="flex-1">
			<button type="button" class="as-btn small blue" onclick="initMemo()"><i class="fa fa-refresh"></i> 선택회원 메모 초기화</button>
		</div>
	</div>
...
function showManageWin(id) {
	window.open('./lotto_member_management.php?mb_id='+id, '', 'width=1024,height=800, scrollbars=yes');
}
...
function highlightBtn(btn) {
	...
	if (property) {
                property.style.backgroundColor = "#dd9200";
        }

        property = document.getElementById(btn);

        property.style.backgroundColor = "#6600FF";
}

var property2;

function highlightBtn2(btn) {

        if (property2) {
                property2.style.backgroundColor = "#dd9200";
        }

        property2 = document.getElementById(btn);

        property2.style.backgroundColor = "#6600FF";
}
...
function changeExtractWeekday() {
        var nos = '';
        var extract_weekday = '';

        extract_weekday = $('#c_mb_extract_weekday').val();
        ...
        if(!nos) {
                alert("발급요일을 변경할 회원을 선택해 주세요.");
                return false;
        }

        if(confirm("선택한 회원들의 발급요일을 변경 하시겠습니까?")) {
                $.ajax({
                        type: "POST",
                        url: "./lotto_member_process.php",
                        dataType: "text",
                        data: {
                                proc:"changeExtractWeekday",
