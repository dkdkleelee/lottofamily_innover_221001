...
?>
	...
	<tr>
		<th><?if(1 || $row['le_send_sms'] == '0') { ?><input type="checkbox" name="chk[]" value="<?=$row['le_no']?>"><? } ?></th>
		<th><?=$idx--?></th>
		<th><?=$row['le_inning']?>회</th>
		<td>
			<img src="/images/ball_<?=sprintf('%02d', $row['le_num1'])?>.png" width="30px" style="<?=$win_data['lw_no'] && !in_array($row['le_num1'], $win_array) ? 'filter: opacity(0.2)' : ''?>">
			<img src="/images/ball_<?=sprintf('%02d', $row['le_num2'])?>.png" width="30px" style="<?=$win_data['lw_no'] && !in_array($row['le_num2'], $win_array) ? 'filter: opacity(0.2)' : ''?>">
			<img src="/images/ball_<?=sprintf('%02d', $row['le_num3'])?>.png" width="30px" style="<?=$win_data['lw_no'] && !in_array($row['le_num3'], $win_array) ? 'filter: opacity(0.2)' : ''?>">
			<img src="/images/ball_<?=sprintf('%02d', $row['le_num4'])?>.png" width="30px" style="<?=$win_data['lw_no'] && !in_array($row['le_num4'], $win_array) ? 'filter: opacity(0.2)' : ''?>">
			<img src="/images/ball_<?=sprintf('%02d', $row['le_num5'])?>.png" width="30px" style="<?=$win_data['lw_no'] && !in_array($row['le_num5'], $win_array) ? 'filter: opacity(0.2)' : ''?>">
			<img src="/images/ball_<?=sprintf('%02d', $row['le_num6'])?>.png" width="30px" style="<?=$win_data['lw_no'] && !in_array($row['le_num6'], $win_array) ? 'filter: opacity(0.2)' : ''?>">
			<? if($row['le_result_grade'] == '2') { ?>
			+ <img src="/images/ball_<?=sprintf('%02d', $win_data['lw_num7'])?>.png" width="30px">
			<? } ?>
		</td>
		<td><?=$lottoService->getNumberType()[$row['le_type']]?><br /><?=$grade_arr[$row['sg_no']]?></td>
...
<!-- 발급번호 삭제 -->
<form name="deleteNumberForm" method="post" action="./lotto_extract_numbers_process.php?<?=$param?>">
