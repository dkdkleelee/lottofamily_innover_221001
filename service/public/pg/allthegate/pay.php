...
$StoreId 	= "tankmart";
$OrdNo 		= $product_buy['sb_no'];
$amt 		= $product_buy['sb_total_price']+$product_buy['sb_vat'];
		...
		if(Check_Common(form) == true){
			...
			if(document.AGSPay == null || document.AGSPay.object == null){
				...
				if(MakePayMessage(form) == true){										
					...
					var openwin = window.open("./pg/allthegate/AGS_progress.html","popup","width=300,height=160"); //"지불처리중"이라는 팝업창연결 부분
...
<form name=frmAGS_pay method=post action=./pg/allthegate/AGS_pay_ing.php>
<input type="hidden" name="sb_no" value="<?=$product_buy['sb_no']?>">
	...
	<!-- 주문번호 (40) -->
		<input type="hidden" style=width:100px name=OrdNo maxlength=40 value="<?=$product_buy['sb_no']?>">
	<!-- 금액 (12) -->
		<input type="hidden" style=width:100px name=Amt maxlength=12 value="<?=$product_buy['sb_total_price']+$product_buy['sb_vat']?>">
		
	
	<!-- 상점명 (50) -->
		<input type="hidden" style=width:300px name=StoreNm value="MS하우징">
	
	<!-- 상품명 (300) -->
		<input type="hidden" style=width:300px name=ProdNm maxlength=300 value="서비스 열람권 - <?=$product_buy['sb_term']?>">
	
	<!-- 상점URL (50) -->
		<!-- 주의) 상점홈페이지주소를 반드시 입력해 주십시오. -->
		<!-- (미입력시 특정 카드사 신용카드 결제 및 가상계좌 결제가 이뤄지지 않을 수 있습니다.) -->
		<input type="hidden" style=width:300px name=MallUrl value="http://msinte.wepas.co.kr">
		<!-- 예) http://www.abc.com -->
	
	<!-- 주문자이메일 (50) -->
		<input type="hidden" style=width:300px name=UserEmail maxlength=50 value="<?=$member['mb_email']?>">
	
	
		<!-- 결제창 좌측상단에 상점의 로고이미지(85 * 38)를 표시할 수 있습니다. -->
		<!-- 잘못된 값을 입력하거나 미입력시 이지스올더게이트의 로고가 표시됩니다. -->
		<!-- 상점로고이미지 URL -->
		<input type="hidden" style=width:400px name=ags_logoimg_url maxlength=200 value="http://msinte.wepas.co.kr/images/main_top_logo.gif">
	
	<!-- 결제창제목입력 -->
		<!-- 제목은 1컨텐츠당 5자 이내이며, 상점명;상품명;결제금액;제공기간; 순으로 입력해 주셔야 합니다. -->
		<!-- 입력 예)업체명;판매상품;계산금액;제공기간; -->
		<input type="hidden" style=width:300px name=SubjectData value="업체명;판매상품;계산금액;<?=$product_buy['sb_term']?>개월;">
		
	<!-- 회원아이디 (20) -->
		<!-- [신용카드, 핸드폰] 결제와 [현금영수증자동발행]을 사용하시는 경우에 반드시 입력해 주시기 바랍니다. -->
		<input type="hidden" style=width:100px name=UserId maxlength=20 value="<?=$_SESSION['ss_mb_id']?>">
	
<!-- + 카드 & 가상계좌 결제 사용 변수 -->

	<!-- 주문자명 (40) -->
		<input type="hidden" style=width:100px name=OrdNm maxlength=40 value="<?=$member['mb_name']?>">
	
	<!-- 주문자연락처 (21) -->
		<input type="hidden" style=width:100px name=OrdPhone maxlength=21 value="<?=$member['mb_tel']?>">
	
	<!-- 주문자주소 (100) --><!-- 가상계좌추가 -->
		<input type="hidden" style=width:300px name=OrdAddr maxlength=100 value="<?=$member['mb_addr1']." ".$member['mb_addr2']?>">
	
	<!-- 수신자명 (40) -->
		<input type="hidden" style=width:100px name=RcpNm maxlength=40 value="<?=$member['mb_name']?>">
	
	<!-- 수신자연락처 (21) -->
		<input type="hidden" style=width:100px name=RcpPhone maxlength=21 value="<?=$member['mb_tel']?>">
	
	<!-- 배송지주소 (100) -->
		<input type="hidden" style=width:300px name=DlvAddr maxlength=100 value="<?=$member['mb_addr1']." ".$member['mb_addr2']?>">
	
	<!-- 기타요구사항 (350) -->
		<input type="hidden" style=width:300px name=Remark maxlength=350 value="">
...
<!-- + 가상계좌 결제 사용 변수 -->
		...
		<input type="hidden" style=width:300px name=MallPage value="/ace_solution/modules/pg/allthegate/AGS_VirAcctResult.php">
...
$('input[name="pay_method"]').on('change', function() {
	switch($(this).val()) {
		case 'card': document.frmAGS_pay.Job.value = 'card'; break;
		case 'iche': document.frmAGS_pay.Job.value = 'iche'; break;
		case 'hp': document.frmAGS_pay.Job.value = 'onlyhp'; break;
		case 'virtual' : document.frmAGS_pay.Job.value = 'virtual'; break;
		default : document.frmAGS_pay.Job.value = ''; break;
	}
});
