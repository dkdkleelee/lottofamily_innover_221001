...
$at_shop_id   = $shop_id;		//설정필요
$at_amt=0;						//결제 금액을 다시 계산해서 만들어야 함(해킹방지), ( session, DB 사용 )

$termService = new TermService();
$row = $termService->getServiceBuy($_SESSION['sb_no']);
$at_amt = $row['sb_price'];
...
if( !strcmp($REPLYCD,"0000") ){
	...
	$result['ORDER_NO']       =getValue("order_no",$at_txt);
	$result['AMT']            =getValue("amt",$at_txt);
	$result['PAY_TYPE']       =getValue("pay_type",$at_txt);
	$result['APPROVAL_YMDHMS']=getValue("approval_ymdhms",$at_txt);
	$result['SEQ_NO']         =getValue("seq_no",$at_txt);
	$result['APPROVAL_NO']    =getValue("approval_no",$at_txt);
	$result['CARD_ID']        =getValue("card_id",$at_txt);
	$result['CARD_NM']        =getValue("card_nm",$at_txt);
	$result['SELL_MM']        =getValue("sell_mm",$at_txt);
	$result['ZEROFEE_YN']     =getValue("zerofee_yn",$at_txt);
	$result['CERT_YN']        =getValue("cert_yn",$at_txt);
	$result['CONTRACT_YN']    =getValue("contract_yn",$at_txt);
	$result['SAVE_AMT']       =getValue("save_amt",$at_txt);
	$result['CARD_POINTDC_AMT']=getValue("card_pointdc_amt",$at_txt);
	$result['BANK_ID']        =getValue("bank_id",$at_txt);
	$result['BANK_NM']        =getValue("bank_nm",$at_txt);
	$result['CASH_BILL_NO']   =getValue("cash_bill_no",$at_txt);
	$result['ESCROW_YN']      =getValue("escrow_yn",$at_txt);
	$result['ACCOUNT_NO']     =getValue("account_no",$at_txt);
	$result['ACCOUNT_NM']     =getValue("account_nm",$at_txt);
	$result['INCOME_ACC_NM']  =getValue("income_account_nm",$at_txt);
	$result['INCOME_LIMIT_YMD']=getValue("income_limit_ymd",$at_txt);
	$result['INCOME_EXPECT_YMD']=getValue("income_expect_ymd",$at_txt);
	$result['CASH_YN']        =getValue("cash_yn",$at_txt);
	$result['HP_ID']          =getValue("hp_id",$at_txt);
	$result['TICKET_ID']      =getValue("ticket_id",$at_txt);
	$result['TICKET_PAY_TYPE']=getValue("ticket_pay_type",$at_txt);
	$result['TICKET_NAME']    =getValue("ticket_nm",$at_txt);	
	$result['PARTCANCEL_YN']  =getValue("partcancel_yn",$at_txt);


	switch($result['PAY_TYPE']) {
		case '3D' :
		case 'ISP' :
		case 'NOR' : $pay_method = 'card'; break;
		case 'ABANK' : $pay_method = 'iche'; break;
		case 'VBANK' : $pay_method = 'virtual'; break;
		case 'HP' : $pay_method = 'HP'; break;
	}
	

?>
<form name="result_from" method="post" action="../service_buy_process.php" onLoad="this.submit();" accept-charset="utf-8">
<input type="hidden" name="proc" value="servicePay">
<input type="hidden" name="pay_method" value="<?=$pay_method?>">
<input type="hidden" name="sb_no" value="<?=$result['ORDER_NO']?>">
<input type="hidden" name="pay_result" value="<?=$result['REPLYCD'] == '0000' ? 'y' : 'n'?>">
...
*/
	...
	echo "결과코드  : ".$REPLYCD."<br>";
	echo "결과메세지: ".iconv('euc-kr', 'utf-8', $REPLYMSG)."<br>";
}
