...
.mem-count li.on, ul.mem-count li.on a {
	...
	<div class="flex m-5">
		<div class="flex-1">
			...
			<button type="button" class="as-btn small green" onclick="stopUsers()"><i class="fa fa-stop"></i> 선택회원 이용중지</button>
    ...
    ?>
    <tr id="pos_<?=$row['mb_no']?>" class="<? if(count($row_service) > 0) { ?>on<? } ?> <?=(!$row['mb_cousult_status'] && !$row_memo[0]['mo_memo']) ? 'not-touched' : ''?>">
		<td>
		...
		<td class="al pl5" <?=($row_memo[0]['mo_memo']) ? 'data-tooltip="<span style=\'font-weight:bold\'>['.$row_memo[0]['mo_datetime']."]</span><br />".nl2br($row_memo[0]['mo_memo']).'"' : ''?>><?=($row_memo[0]['mo_memo']) ? '<span >'.cut_str($row_memo[0]['mo_memo'],45)."</span><br /><span style='font-weight:bold'>[".$row_memo[0]['mo_datetime']."]</span>"  : '상담이전'?></td>
        <td>
