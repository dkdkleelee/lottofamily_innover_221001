...
<select name="s_mb_charger_tm" class="frm_input">
	...
	<tr>
		<td><?=$data['idx']--?></td>
		<td><a href="javascript:showManageWin('<?=$row['mb_id']?>');void(0);"><?=$row['sb_buyer_name']?>(<?=($row['mb_id']) ? $row['mb_id'] : '<span style="color:red;font-weight:bold">비회원</span>'?>)</a></td>
		...
		<td>
    		<? if($row['sb_pay_status'] == 'N' && $row['sb_no'] != '479') { ?>
