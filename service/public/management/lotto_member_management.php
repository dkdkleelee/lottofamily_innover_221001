...
$param1 = Utils::getParameters(array('page','url','nocache'));
...
// 메모팝업의 경우
if($_GET['mo_no']) {
	$user->updateMemoDone($_GET['mo_no']);
}
...
<div class="content_wrap">
	...
	<tr>
		...
		<td width="40%">
			...
			<button type="button" class="as-btn small blue" onclick="addNumbersToQueue()"><i class="fa fa-send"></i> 추가발급</button>
		</td> -->
	</tr>
	<tr>
	...
	<div style="width:100%;text-align:center;padding-bottom:30px">
		<button type="button" onclick="document.member_form1.submit()" class="as-btn small blue"><i class="fa fa-check"></i> 확인</button>
		<button type="button" class="as-btn small red" onclick="window.close();void(0);"><i class="fa fa-close"></i> 닫기</button>
	</div>
	</form>

	<form name="member_form2" id="member_form2" method="POST" encType="multipart/form-data" action="./lotto_member_process.php" onsubmit="return formSubmit(this);">
			...
			<div style="width:100%;height:200px;overflow-y:scroll;border:1px solid #eeeeee;margin-top:5px">
				...
				?>
					<li style="padding:5px;border-bottom:1px dotted #888888">
						<?=nl2br($value['mo_memo'])?><div style="color:#6666ff"><?=$value['mo_datetime']?> [<span style="font-weight:bold"><?=$value['mb_name']?></span>] <?=$value['mo_schedule'] ? '<span style="padding-left:50px"><i class="fa fa-bell"></i> '.$value['mo_schedule_datetime']."</span>" : ''?></div>
	...
	<div style="width:100%;text-align:center;padding-top:30px">
		<button type="button" class="as-btn small blue" onclick="document.member_form2.submit()"><i class="fa fa-check"></i> 확인</button>
		<button type="button" class="as-btn small red" onclick="window.close();void(0);"><i class="fa fa-close"></i> 닫기</button>
	</div>
	</form>
	<!--// 회원기본정보 및 상담정보 -->
...
<? if($data['mb_id']) { ?>
	...
	<tr>
		<td>
			...
			결제금액: <input type="text" name="new_price" id="new_price" placeholder="숫자만 입력" class="frm_input" style="width:150px;margin:2px">원
			<!-- <br />
	...
	<tr>
		...
		<th width="90px">신청일</th>
		<th width="90px">결제일</th>
		<th>서비스</th>
	...
	<tr>
		...
		<td id="pay_status_<?=$row['sb_no']?>">
			...
			<? if($row['sb_pay_method'] == 'agency' && $row['sb_pay_status'] == 'N') { ?>
				<!-- <button type="button" class="as-btn tiny blue" onclick="agencyPay('<?=$row['sb_no']?>')"><i class="fa fa-credit-card"></i> 결제진행</button> -->
				<!-- <button type="button" class="as-btn tiny blue" style="" onclick="agencyPay3('<?=$row['sb_no']?>','futureSurf')"><i class="fa fa-credit-card"></i> FS결재진행(사용금지)</button> -->
				<!--button type="button" class="as-btn tiny blue" style="" onclick="agencyPay3('<?=$row['sb_no']?>','payUp')"><i class="fa fa-credit-card"></i> PU결제진행</button-->
				<!--button type="button" class="as-btn tiny blue" onclick="agencyPay3('<?=$row['sb_no']?>','SHCard')"><i class="fa fa-credit-card"></i> SHC결제진행</button-->
                                <button type="button" class="as-btn tiny blue" onclick="agencyPay3('<?=$row['sb_no']?>','SHCard2')"><i class="fa fa-credit-card"></i> AB결제진행</buttons>
			<? } else if($row['sb_ay_pg'] == 'payUp' && $row['sb_pay_status'] == 'Y') { ?>
			
				<button type="button" class="as-btn tiny blue" style="" onclick="agencyPayCancel('<?=$row['sb_no']?>','payUp')"><i class="fa fa-credit-card"></i> PU결재취소</button>
			<? } ?>
		</td>
		<td >
    		<?=$row['sb_pay_name']?>
	...
	?>
		<form name="serviceUseForm_<?=$row['su_no']?>" method="post" action="./lotto_member_process.php">
...
function openUserWinList(mb_id) {
	window.open("./lotto_member_win_list.php?id="+mb_id, 'win_list', 'width=950, height=800, scrollbars=yes');
}
