        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                if ($this->is_valid_image_file($file_path)) {
					...
					$_app = new App();
					if($_app->config_default['cf_watermarkImage']) {
						$manager = new ImageManager(array('driver' => 'gd'));
						$image = $manager->make($file_path);
						
						$watermark = $manager->make($_app->getUploadPath() . "/config/".$_app->config_default['cf_watermarkImage'])->opacity(50);
						$image->insert($watermark, 'bottom-right', 10, 10)->save($file_path);
					}
