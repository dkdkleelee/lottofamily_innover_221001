	...
	public function getUserListWithService($page=1, $url='') {
		...
		if($_GET['sc'] && $_GET['sv']) {
			switch($_GET['sc']) {
				...
				case 'mo_memo' :
					// 메모검색 조인
					$this->db->join("(SELECT m.mb_id, m.mo_memo FROM ".$this->tb['Memo']." as m WHERE mo_memo LIKE '%".$_GET['sv']."%' group by m.mb_id) as d", "d.mb_id=a.mb_id", "LEFT");
	...
	public function getUserListWithServiceForTM($page=1, $url='') {
		...
		if($_GET['sc'] && $_GET['sv']) {
			switch($_GET['sc']) {
				...
				case 'mo_memo' :
					// 메모검색 조인
					$this->db->join("(SELECT m.mb_id, m.mo_memo FROM ".$this->tb['Memo']." as m WHERE mo_memo LIKE '%".$this->db->escape($_GET['sv'])."%' group by m.mb_id) as d", "d.mb_id=a.mb_id", "LEFT");
					$this->db->where("d.mb_id is not NULL");
					break;
...
/*
				if(isset($_GET['s_sg_no']) && $_GET['s_sg_no']) {
						...
						$this->db->orderBy('mb_datetime', 'DESC');
                                		...
                                		$this->db->orderBy('x.su_startdate', 'DESC');
                        		}
                		} else {
					$this->db->orderBy('mb_datetime', 'DESC');
		                }				
...
public function getUserListWithServiceForTM1($page=1, $url='') {
                global $pageLimit;
		...
		if($_GET['s_lwr_grade']) {
                ...
                $this->db->join("(SELECT x.mb_id, x.su_enddate, x.su_pausedate, x.su_no, x.sg_no, y.sg_name FROM ".$this->tb['TermServiceUse']." as x LEFT JOIN ".$this->tb['TermServiceGrade']." as y ON(x.sg_no=y.sg_no) WHERE su_enddate >= NOW() $join_group AND x.sg_no <= 3) as b", "a.mb_id=b.mb_id", "LEFT");
...
public function getUserListWithNoLogin ($page=1, $url='', $le_inning, $lwr_grade) {
     ...
     if($_GET['s_nologin']) {
		...
		*/
		...
		//if($_GET['s_nologin']) {
		if ($lwr_grade != 4) {
			$this->db->where("(mb_today_login < le_issued_date OR le_issued_date IS NULL) AND (c.cnt = 0 OR c.cnt IS NULL) AND e.cnt_inning != 0 AND b.sg_no IS NULL AND NOT a.mb_cousult_status IN ('정지') AND a.mb_status = 1");
		}
		else {
			$this->db->where("(mb_today_login < le_issued_date OR le_issued_date IS NULL) AND (c.cnt <= 1 OR c.cnt IS NULL) AND e.cnt_inning != 0 AND b.sg_no IS NULL AND NOT a.mb_cousult_status IN ('정지') AND a.mb_status = 1");
		}
		//$this->db->where('b.sg_no IS NULL OR (NOT c.lwr_grade IN (1,2,3))');
			//$this->db->where("c.le_result_grade <= 3 AND c.le_result_grade >= 1");	
		//}
		...
		// 2021-07-31 수정본- lotto_numbers_win 으로 처리
		$this->db->join("(SELECT mb_id, COUNT(IF(lwr_grade = ".$lwr_grade.", mb_id, null)) as cnt FROM ".$this->tb['LottoNumbersWin']." GROUP BY mb_id) as c", "c.mb_id=a.mb_id", "LEFT");

		// 2021-07-31 원본- DB삭제후 적용어려운 구조.
...
//			'mb_update_date' => $this->db->now()
	...
	public function updateMemoTM($mb_id, $mb_tm_id, $mo_memo) {

		if ($mo_memo == null) return;
	...
	public function getSimul($mb_id) {
		
		$this->db->where("mb_id", $mb_id);
                $row = $this->db->arraybuilder()->getOne('g5_write_photo2', "wr_content");

		return $row["wr_content"];
	}

	public function deleteSimul($mb_id) {
		
		$this->db->where("mb_id", $mb_id);
                $row = $this->db->arraybuilder()->getOne('g5_write_photo2', "wr_content");
		if ($row["wr_content"] != null) {
			$query = "delete from g5_write_photo2 where mb_id = '".$mb_id."'";
                	$this->db->rawQuery($query);
			
			$dist_path = $this->config['upload_path'] . "/file/simul";
			unlink($dist_path."/".$row["wr_content"]);
		}
	
	}
	
	public function uploadSimul($mb_id, $simul_img) {
		

		if(isset($_FILES['simul_img']) && $_FILES['simul_img']['name'] != "") {

			//업로드 경로 지정
			$dist_path = $this->config['upload_path'] . "/file/simul";

			$file = $_FILES['simul_img'];
			$ext_str = "jpg,png,JPG,PNG";
			...
			if ($row["wr_content"] != null) {
				...
				$this->db->rawQuery($query);
                	...
                	if($_FILES['simul_img']['size'] > 0) {
            			if (!is_dir($dist_path)) mkdir($dist_path, 0707);
                        	$file = $_POST['mb_id'] . "_simul_" . basename($_FILES['simul_img']['name']);
            			$uploaded_file = Utils::uploadFile('simul_img', $dist_path . "/"  . $file);

            			$data['simul_img'] = basename($uploaded_file['destFile']);

                        	if(is_file($dist_path."/".$data_ori['simul_img'])) {
                                	@unlink($dist_path."/".$data_ori['simul_img']);
                        	}
        		}

			$query = "INSERT INTO g5_write_photo2 (mb_id, wr_content, wr_datetime) VALUES('".$mb_id."','".$file."', now())";	
			$this->db->rawQuery($query);
...
if (!$num7 || $num7 < 1 || $num7 > 45 || !is_numeric($num7)) { echo "not number 7 ball<br>\n"; return false; }
	...
	public function deleteMemoById($mb_id) {
		...
		$this->db->where('mb_id', $mb_id);
                $this->db->delete($this->tb['SMSHistory']);


		// 상담상태초기화
		$data['mb_name'] = $mb_id;
		$data['mb_nick'] = $mb_id;
		$data['mb_nick_date'] = $this->db->NOW();
		$data['mb_today_login'] = $this->db->NOW();
		$data['mb_datetime'] = $this->db->NOW();
		$data['mb_open_date'] = $this->db->NOW();
		$data['mb_update_date'] = $this->db->NOW();
	...
	public function excelNewUserUpload() {
		...
		for($i=1; $i<count($rows); $i++) {
			...
			if(trim($rows[$i][3])) { // 휴대전화가 있는데이터만
				...
				//$data_row['8'] = isset($data_row['8']) ? '' : $data_row['8'];
	...
	public function getChannelGroup($tm_id = '', $add_condition='') {
		...
		$rows = $this->db->get($this->tb['Member'], null, " count(*) as cnt, IF(mb_channel IS NULL, '', mb_channel) as mb_channel");
