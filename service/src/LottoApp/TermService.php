	...
	function getTermServiceBuyList($page=1, $url='') {
		...
		if($_GET['s_mb_charger_tm'] != '') {
                        if($_GET['s_mb_charger_tm'] == 'na') {
                                $this->db->where("(a.sb_tm_id IS NULL OR a.sb_tm_id = '')");
                        } else {
                                $this->db->where('a.sb_tm_id', $_GET['s_mb_charger_tm']);
                        }
                }
...
*/
	...
	function downloadServiceBuyExcel() {
        ...
        function SaveSendSMS ($wr_no, $reply, $content, $wr_booking, $countDest, $wr_success) {
		//alert($content);
                $this->db->rawQuery("insert into ".$this->tb['SMSWrite']." set wr_no='$wr_no', wr_renum=0, wr_reply='$reply', wr_message='$content', wr_booking='$wr_booking', wr_total='$countDest', wr_datetime='".G5_TIME_YMDHIS."', wr_success='$wr_success'");
        }

        function OKSendSMS ($wr_no, $mb_id, $mb_name, $hp, $hs_memo) {

                sql_query("insert into ".$this->tb['SMSHistory']." set wr_no='$wr_no', wr_renum=0, bg_no=0, mb_id='$mb_id', bk_no=0, hs_name='$mb_name', hs_hp='$hp', hs_datetime='".G5_TIME_YMDHIS."', hs_flag='1', hs_code='0', hs_memo='$hs_memo', hs_log=''", false);
        }
