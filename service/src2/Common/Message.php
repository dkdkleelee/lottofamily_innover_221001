		...
		if(1 || $member_row['mb_sms']) { // 메세지는 SMS 수신여부와 상관없이 보냄
			...
			if($result) {
				...
				if($result) {
					...
					$this->db->rawQuery("insert into {$g5['sms5_write_table']} set wr_no='$wr_no', wr_renum=0, wr_reply='$reply', wr_message='".$msg['message']."', wr_booking='$wr_booking', wr_total='".count($dest)."', wr_datetime='".G5_TIME_YMDHIS."'");
