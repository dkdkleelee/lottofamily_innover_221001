...
class LottoService extends Base
{
	...
	function addSelectedNumbersToQueue($mb_id, $nos) {
		...
		$title = "[".$this->site_conf['name']."/".$this_inning."] ".$mb['mb_id']."(".$mb['mb_name'].")님 자동발급 ".count($nos)."개";
	...
	function addNumbersToQueue($mb_id, $issue_count=0, $send_msg=1) {
		...
		$this->extractNumbers('', '', $issue_count*10);
		///////////////////////////////////////////////
		...
		$title = "[".$this->site_conf['name']."/".$this_inning."] ".$mb['mb_id']."(".$mb['mb_name'].")님 자동발급 ".$issue_count."개";
	...
	function extractNumbers($config='', $mb_id='', $count=1) {
	...
	public function checkResult($inning='', $le_no='', $recheck=false) {
		...
		if($row_win['lw_inning'] && ($row['cnt'] == 0 || $le_no != '' || $recheck === true)) {
			...
			if(is_array($result_data)) {
				...
				if(($le_no == '' && $recheck === false)) { // 강제재확인 및 번호업데이트가 아닌 최초 확인시에만
					...
					for($i=0; $i<count($result_data); $i++) {
						if($result_data[$i]['mb_id']) {
							...
							$title = "[".$this->site_conf['name']."] ".$mb['mb_name']."(".$result_data[$i]['mb_id'].")님 ".$result_data[$i]['lwr_inning']."회 ".$result_data[$i]['lwr_grade']."등 당첨 축하드립니다.";
							//$content = "[".$this->site_conf['name']."] ".$mb['mb_name']."(".$result_data[$i]['mb_id'].")님 ".$result_data[$i]['lwr_inning']."회 ".$result_data[$i]['lwr_grade']."등 당첨 축하드립니다.";
							$content = $mb['mb_name']."(".$result_data[$i]['mb_id'].")님 이번 ".$result_data[$i]['lwr_inning']."회 추출번호에서 ".$result_data[$i]['lwr_grade']."등 당첨나왔습니다. 지금바로 확인해주세요.\n";
							$content .= "-".$this->site_conf['name']."-";
	...
	public function getExtractNumberList($page=1, $url='', $pageLimit=20) {
		...
		if($_GET['s_sg_no']) {
			if($_GET['s_sg_no'] == 'normal') {
				$this->db->where('c.sg_no IS NULL');
			} else {
				$this->db->where('c.sg_no', $this->db->escape($_GET['s_sg_no']));
			}
		}
		...
		$list = $this->db->arraybuilder()->paginate($this->tb['LottoNumbers']." as a ", $page, "a.*, c.sg_no, c.su_enddate, b.mb_name, b.mb_today_login, IF(a.le_issued_date > b.mb_today_login, 0, 1) as varified");
	...
	///////////////////////////////////////////////////////////////////////////////
	// test 회원에게 지정된 갯수만큼 발급 테스트 - /public/admin/test.php
	///////////////////////////////////////////////////////////////////////////////
	function setNumbersToUserTEST() {
		// 1. 회원별 발급갯수 조회
		// 관리자 추출설정 및 서비스설정
		$serviceConfig = $this->lottoServiceConfig->getConfig();
		...
		//$this->db->where("(a.mb_extract_weekday='".$cur_weekday."')");
		$this->db->where("a.mb_sms='1'");
		$this->db->where("a.mb_id='test'"); // 회원아이디 test 고정
		$this->db->join($this->tb['TermServiceUse']." as b", "a.mb_id=b.mb_id", "LEFT");
		$list = $this->db->arraybuilder()->get($this->tb['Member']." as a ", $page, "a.*, b.su_no, b.sg_no, b.su_enddate, 
				IF(DATEDIFF(su_enddate, now()) > 0, DATEDIFF(su_enddate, now()), 0) as leftDays, 
				(select count(*) FROM ".$this->tb['LottoNumbers']." as c WHERE c.mb_id=a.mb_id AND le_inning='".$this_inning."' AND le_type='extractor') as issued_count ");
		...
		// 2. 지정갯수별 발급
		{
			...
			for($i=0; $i<count($list); $i++) {
				...
				for($j=0; $j<$issue_count; $j++) {
					...
					$content .= $list_number['le_num1']." ".$list_number['le_num2']." ".$list_number['le_num3']." ".$list_number['le_num4']." ".$list_number['le_num5']." ".$list_number['le_num6']."\n";
...
exit;
				...
				if($issue_count > 0) {
					...
					// 해당번호 발급상태로 업데이트
					$this->db->where('le_no', $ids, 'IN');
					$this->db->update($this->tb['LottoNumbers'], ['mb_id' => $list[$i]['mb_id'], 'le_issued_date' => $this->db->NOW()]);
