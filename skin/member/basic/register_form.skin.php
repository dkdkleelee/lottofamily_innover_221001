...
if($_SESSION['auth_hp'] && !$member['mb_hp']) {
	$member['mb_hp'] = $_SESSION['auth_hp'];
	$member['mb_extract_weekday'] = $send_weekday;
}
//echo var_dump(!valid_mb_nick("121212*****"));
?>
<? include "../head_07.php" ?>
<!-- 회원정보 입력/수정 시작 { -->
<div class="mbskin">
	<link rel="stylesheet" href="/ace_solution/public/css/custom.style.css" type="text/css">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
        <tr>
            <th scope="row" style="width:150px"><label for="reg_mb_id">아이디<strong class="sound_only">필수</strong></label></th>
            <td>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
        <tr>
            <th scope="row" style="width:150px"><label for="reg_mb_name">이름<strong class="sound_only">필수</strong></label></th>
            <td>
        ...
        <?php if ($req_nick || !check_string($member['mb_nick'], G5_HANGUL + G5_ALPHABETIC + G5_NUMERIC)) {  ?>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="mb_email" value="<?php echo isset($member['mb_email'])?$member['mb_email']:''; ?>" id="reg_mb_email" class="frm_input email " size="70" maxlength="100">
            </td>
        </tr>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
			<input type="hidden" name="mb_extract_weekday" value="<?=($member['mb_extract_weekday'] ? $member['mb_extract_weekday'] : '4')?>">
		<!-- <tr>
			<th width="100px">문자발송요일</th>
			<td>
				<? for($i=1; $i<6; $i++) { ?>
				<label><input type="radio" name="mb_extract_weekday" value="<?=$i?>" <?=$member['mb_extract_weekday'] == $i ? 'checked' : ''?>><?=$termService->config['weekdays_name'][$i]?>&nbsp;&nbsp;&nbsp;</label>
				<? } ?>
			</td>
			
		</tr>
		<tr>
			<th width="100px">주당 평균 구매게임 수</th>
			<td>
				<select name="mb_average_game_number" id="mb_average_game_number" class="frm_input">
					<option value="5">5게임 </option>
					<option value="10">10게임</option>
					<option value="15">15게임</option>
					<option value="20">20게임</option>
					<option value="25">25게임</option>
					<option value="30">30게임</option>
					<option value="35">35게임</option>
					<option value="40">40게임</option>
					<option value="45">45게임</option>
					<option value="50">50게임</option>
					<option value="70">70게임</option>
					<option value="90">90게임</option>
					<option value="100">100게임</option>
				</select>
				<script>
					var mb_average_game_number = '<?=$member['mb_average_game_number']?>';
					if(mb_average_game_number) {
						$('#mb_average_game_number').val(mb_average_game_number).prop("selected", true);
					}
				</script>
			</td>
		</tr> -->
        ...
        <tr>
            <th scope="row" style="width:150px"><label for="reg_mb_signature">서명<?php if ($config['cf_req_signature']){ ?><strong class="sound_only">필수</strong><?php } ?></label></th>
        ...
        <tr>
            <th scope="row" style="width:150px"><label for="reg_mb_mailling">메일링서비스</label></th>
            <td>
        ...
        <tr>
            <th scope="row" style="width:150px"><label for="reg_mb_sms">SMS 수신여부</label></th>
            <td>
				<? if($member['mb_sms']) { ?>
        ...
        <tr>
            <th scope="row" style="width:150px">자동등록방지</th>
